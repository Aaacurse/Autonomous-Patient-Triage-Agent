...
async def intake_node(state: TriageState) -> TriageState:
    ...
    if not raw:
        return {**state, "last_node": "intake", "error": "Empty complaint recieved"}

    return {**state, "last_node": "intake", "raw_complaint": raw.lower(), "error": None}
...
async def nlp_extract_node(state: TriageState, config: RunnableConfig) -> TriageState:
    ...
    if db and mrn:
        history_summary, repeat_high_acuity_visit = await maybe_fetch_patient_history(
            complaint, db, mrn
        )
...
async def disposition_node(state: TriageState) -> TriageState:
    ...
    routing = {2: "immediate_care", 3: "acute_care", 4: "fast_track", 5: "waiting_room"}
