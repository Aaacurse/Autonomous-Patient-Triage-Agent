...
class NLPExtractionOutput(BaseModel):
    ...
    is_life_threat: bool = Field(
        description="True if airway/breathing/circulation is compromised or patient is unresponsive"
    )
    is_high_risk: bool = Field(
        description="True if severe pain 7+, altered mental status, or high-risk presentation"
    )
    estimated_resources: list[str] = Field(
        description="ED resources needed: labs, imaging, iv_medication, consult"
    )
    danger_zone_vitals: bool = Field(
        description="True if any vitals are in danger zone"
    )
    clinical_notes: str = Field(description="One sentence clinical summary")
    inferred_pain_score: int = Field(
        description="Pain score 0-10 inferred from description. 0=none, 1-3=mild, 4-6=moderate, 7-9=severe, 10=worst possible. Use qualitative words if no numeric score given."
    )
    repeat_high_acuity_visit: bool = Field(
        description="True only if patient triage history was provided AND it shows a prior ESI 1 or 2 visit, or a pattern of escalating severity relevant to the current complaint. Must be false if no history was provided."
    )
...
async def maybe_fetch_patient_history(
    complaint: str, db, mrn: str
) -> tuple[str | None, bool]:
    """
...
async def groom_with_groq(
    complaint: str, scispacy_entities: list[dict], history_summary: str | None = None
) -> NLPExtractionOutput:
